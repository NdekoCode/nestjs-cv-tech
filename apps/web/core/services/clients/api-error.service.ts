...
import { ApiErrorResponse } from '@/core/types/services';
...
export const AxiosErrorDetails: Record<string, AxiosErrorInfo> = {
  ...
  ERR_NETWORK: {
    label: "Network Error",
    description:
      "The network is inaccessible or the request has not reached the server.",
...
export class ApiErrorService extends Error {
   ...
   */
  static fromAxiosError<T = any>(
    error: T | AxiosError<ApiErrorResponse>
  ): ApiErrorService {
    ...
    const extractStatus =
      extractHttpCode(errorMessage.trim().split("HTTP")?.[1]) ?? 500;

    let statusCode = errorFrom.status ?? response?.status ?? extractStatus;
    const info =
      Object.values(AxiosErrorDetails).find(
        (value) => value.httpCode === errorStatus
      ) || AxiosErrorDetails[errorCode!];
    if (info) {
      errorMessage = `Error (${info.label}) – ${info.description} (HTTP ${
        info.httpCode || "unknown"
      })`;
